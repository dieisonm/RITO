...
related_ids: ["20260702-fluxo-canonico-de-codigo-arquivos-e-deploy-do-relatorio-de-vendas"]
issue: ""
pr: ""
commit: ""
---
...
Decisoes tecnicas importantes: builds de producao usam `next build --webpack`; Turbopack causou erro de chunks no runtime OpenNext. PDFs sao lidos diretamente com `pdfjs-dist/legacy`. O deploy OpenNext no Windows pode depender de fallback local para falha de symlink em `@opennextjs/aws`; isso deve ser revalidado antes de futuros deploys. A instalacao local foi reconstruida com `npm ci` no S e lint, TypeScript e build passaram sem erros.
